...
| `GET` | `/api/tickets/{id}/download` | 🔒 USER | Скачать электронный билет в формате PDF с QR-кодом | — | PDF файл (200) | ✅ |
...
| `POST` | `/api/admin/checkin/{token}` | 🔒 ADMIN | Сканирование QR-кода на входе. Сверяет токен, меняет статус билета на USED | — | `{message, ticketId, customerName, seatLabel}` | ✅ |
